...
public class NetworkServiceImpl implements NetworkServiceExt {
	...
	private NetworkServiceImpl() {
		...
		System.out.println("AMLOG:: user name:: " + details);
			userSession = createUserSession(System.getProperty("user.name"));
		System.out.println("AMLOG:: user session id " + userSession.userSession.getId());
	}
...
//	added by amjad
	...
	public UserSession createUserSession(String userName) {
		...
		try {
			final UserSession 	userSession = new UserSession(new URI("ws://localhost:8080/ws/echo/" + userName));
			userSession.addMessageHandler(new UserSession.MessageHandler() {
